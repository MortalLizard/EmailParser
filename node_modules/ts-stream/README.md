...
-   Add `batch()` transform (#49, thanks @MattiasMertens)
    -   Fixed a flush timeout bug in batch (#57, thanks @MarcWeber)
-   Add `endCatcher()` transform (#53, thanks @MattiasMertens)
-   Removed dependency on NodeJS `assert` module for better browser support (#57, thanks @MarcWeber)
-   Breaking change: Don't return error from ender as stream's result when explicit result
    promise was passed to end() (#51)
    -   This allows e.g. marking a stream's final result as OK, even if an ender callback threw
        an error.
    -   This only affects you if you explicitly pass a second argument to `end()`, and then only
        if an ender callback throws an error.
    -   See the description in #51 and updated test cases in commit 18ae35e6fa89ecc5830186c1f62993bb13bebd69
        for details.
